...
#define GROWTHCOIN_SEED_NONCE  0x0539a019ca550825ULL
...
//    printf("%s: SEND %s\n", ToString(you).c_str(), pszCommand);
  ...
  void PushVersion() {
    ...
    uint64 nLocalNonce = GROWTHCOIN_SEED_NONCE;
    int64 nLocalServices = 0;
    CAddress me(CService("0.0.0.0"));
    BeginMessage("version");
    int nBestHeight = GetRequireHeight();
    string ver = "/growthcoin-seeder:0.1.2/";
...
/*
int main(void) {
  CService ip("seed.growthco.in", 7112, true);
  vector<CAddress> vAddr;
  vAddr.clear();
  int ban = 0;
